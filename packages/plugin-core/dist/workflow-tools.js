...
import { parseSpecRefs, createValidTask, } from './bd';
                ...
                if (markdownDrafts.length === 1) {
                    ...
                    if (askFn) {
                        const answer = await askFn({
                            ...
                            options: markdownDrafts.map((d) => ({
                                label: d.replace('.md', ''),
                                description: `Convert ${d} to OpenSpec documents`,
                            })),
                        });
                        draftName = answer?.choice;
                    }
                    if (!draftName) {
                ...
                if (changeDirs.length === 1) {
                    ...
                    if (askFn) {
                        const answer = await askFn({
                            ...
                            options: changeDirs.map((d) => ({
                                label: d,
                                description: `Create implementation plan for ${d}`,
                            })),
                        });
                        specName = answer?.choice;
                    }
                    if (!specName) {
                ...
                if (markdownPlans.length === 1) {
                    ...
                    if (askFn) {
                        const answer = await askFn({
                            ...
                            options: markdownPlans.map((p) => ({
                                label: p,
                                description: `Create tasks from ${p} plan`,
                            })),
                        });
                        planName = answer?.choice;
                    }
                    if (!planName) {
...
Output your review in the format specified in your prompt (通过审查 or 需修改).`,
        ...
        const needsModification = reviewReport.includes('需修改') ||
